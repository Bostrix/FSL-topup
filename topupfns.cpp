...
: _imain(imain.value()), _out(out.value()), _fout(fout.value()), _iout(iout.value()), _dfout(dfout.value()),
  ...
  _regrid((regrid.value()==0) ? false : true), _nthr(static_cast<unsigned int>(nthreads.value())), _estmov(estmov.value()), 
  ...
  _profile(profile.value())
{
  ...
  // --nthr
  if (nthreads.value() != 1) { // If multi-threaded run has been requested
    unsigned int ncores = std::thread::hardware_concurrency();
    if (ncores == 0 || nthreads.value() <= ncores) { // If no info on no of cores, go with what user says
      _nthreads = nthreads.value();
    }
    else _nthreads = ncores;
  }
  else _nthreads = 1;    
}
                                                      ...
                                                      char           *args[])
{
  ...
  Utilities::Option<int> nthreads(string("--nthr"),1,
      string("\tNumber of threads to use (cannot be greater than numbers of hardware cores), default 1"),false,Utilities::requires_argument);
  ...
  Utilities::HiddenOption<bool> traceprint(string("--trace"),false,
      string(""), false, Utilities::no_argument);

  Utilities::HiddenOption<bool> profile(string("--profile"),false,
      string(""), false, Utilities::no_argument);
  ...
  try {
    clp = std::shared_ptr<topup_clp>(new topup_clp(imaname,dataname,outname,foutname,ioutname,dfoutname,rbmoutname,
						     ...
						     interpolation,individualscaling,regridding,nthreads,verbose,debug,traceprint,profile));
  }
  catch(std::exception& e) {
    options.usage();
