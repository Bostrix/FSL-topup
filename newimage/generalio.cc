...
//Allowed image extensions. The first extension is the header-bearing file,
//the second the data-bearing file. For non-paired images these are the same,
//but the consistent format can make automated extension processing easier.
const std::array<std::pair<const string, const string>, 4> extensions = { {
  { ".nii.gz", ".nii.gz" },
  { ".nii", ".nii" },
  { ".hdr", ".img" },
  { ".hdr.gz", ".img.gz" }
		  ...
		  const bool readAs4D)
{
  ...
  int64_t nthreads = target.nthreads();
  target.destroy();
  ...
  if ( ! ( getenv("FSL_LOAD_NIFTI_EXTENSIONS") && atoi(getenv("FSL_LOAD_NIFTI_EXTENSIONS")) != 0 ) ) {
  ...
  if (tbuffer==NULL)
    cout << "help" << endl;
  target.initialize(header.dim[1],header.dim[2],header.dim[3],header.dim[4],header.dim[5],header.dim[6],header.dim[7],tbuffer,true,nthreads);
  // copy info from file
...
//This removes a _single_ image extension ( if present ) from a file name
//foo.nii.img.hdr.nii.gz will become foo.nii.img.hdr
std::string make_basename(std::string &filename) //note passed as reference for compat with old API - use as a return type is preferred
{
  for(const auto& extension : extensions) {
    if (filename.length() >= extension.first.length() && filename.compare(filename.length() - extension.first.length(), extension.first.length(), extension.first) == 0) {
      filename.erase(filename.length() - extension.first.length(), extension.first.length());
      break;
    }
    if (filename.length() >= extension.second.length() && filename.compare(filename.length() - extension.second.length(), extension.second.length(), extension.second) == 0) {
      filename.erase(filename.length() - extension.second.length(), extension.second.length());
      break;
    }
  }
  return filename;
...
{
  return filesystem::is_regular_file(filesystem::path(filename));
}

//If input filename has a known image extension then return the associated
//header-containing filename - the same file for non-paired images.
//Otherwise append image extensions to filename and check for a single,
//unique match on the filesystem
string return_validimagefilename(const string& filename, const bool quiet, const bool strict) {
  string bname(make_basename(filename));
  string validname;
  int validcount=0;
  if (bname != filename) {
    //Deduce removed extension and return header-containing file
    for(const auto& extension : extensions)
      if ( bname+extension.first == filename || bname+extension.second == filename )
        validname = bname + extension.first;

    if (valid_imagefilename(validname))
      return validname; //If the original filename had a full image extension and exists, return it.
    else if ( strict )
  ...
  for(const auto& extension : extensions)
    if (valid_imagefilename(bname+extension.first)) {
      validname=bname+extension.first;
      validcount++;
    }
  if (validcount>1)
